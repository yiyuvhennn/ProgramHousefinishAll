...
function buildHouseFromBody(body) {
    return {
...
function mapHouseDbToApi(h) {
    return {
        ...
        features: (h.houseFeatures ?? [])
            .map((hf) => hf?.feature?.name)
            .filter(Boolean),
    };
}
function prismaUniqueError(e) {
    return e?.code === "P2002";
}
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        if (slug) {
            const rows = await prisma_1.prisma.house.findMany({
                where: { slug },
...
router.post("/", async (req, res) => {
    ...
    try {
        const created = await prisma_1.prisma.house.create({
            data: {
                slug: data.slug,
                title: data.title,
                city: data.city,
...
router.patch("/:id", async (req, res) => {
    ...
    if (body.slug !== undefined)
        updateData.slug = toStringSafe(body.slug);
    if (body.title !== undefined)
        updateData.title = toStringSafe(body.title);
    if (body.city !== undefined)
        updateData.city = toStringSafe(body.city);
    if (body.price !== undefined)
        updateData.price = toStringSafe(body.price);
    if (body.area !== undefined)
        updateData.area = toStringSafe(body.area);
    if (body.rooms !== undefined)
        updateData.rooms = toStringSafe(body.rooms);
    if (body.image !== undefined)
        updateData.image = toStringSafe(body.image);
    if (body.description !== undefined)
        updateData.description = toStringSafe(body.description);
    const hasFeaturesUpdate = body.features !== undefined;
    const nextFeatures = hasFeaturesUpdate ? parseFeatures(body.features) : [];
    try {
        // 先確認存在
        const exists = await prisma_1.prisma.house.findUnique({ where: { id } });
        if (!exists)
...
router.delete("/:id", async (req, res) => {
    ...
    if (!Number.isFinite(id))
        return res.status(400).json({ message: "Invalid id" });
    try {
        await prisma_1.prisma.house.delete({ where: { id } });
